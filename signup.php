...
if ($_SERVER['REQUEST_METHOD']=="POST") {
  if (empty($_POST['username']) || empty($_POST['email']) || empty($_POST['password']) || empty($_POST['cpassword'])) {
    ...
    $customer_cpassword = $_POST['cpassword'];
    $country_id = $_POST['country_id'];
    if (isExists("customer_username", $customer_username)) {
      $_SESSION['errors']['username'] = "Username already exists!";
    }
    if (isExists("customer_email", $customer_email)) {
      $_SESSION['errors']['email'] = "Email already exists!";
    }
    if ($customer_password!=$customer_cpassword) {
      $_SESSION['errors']['password'] = "Password & Confirm Password mismatched!";
    }

    if ($_SESSION['errors']==null) {
      $accountCreated = createCustomerAccount([
      ...
      if ($accountCreated) {
        ...
        $_SESSION['errorsDB'] = "Failed to create account!";
      }
    }
  }
}
           ...
           <div class="content">
               ...
               if ($_SESSION['errors']!=null) {
                foreach ($_SESSION['errors'] as $errKey => $errValue) {
                  echo "<span class='text-danger'><b>".ucfirst($errKey).":</b> $errValue </span><br>";
                }
                echo "<br>";
                $_SESSION['errors']=null;
               }
               ...
               if (isset($_SESSION['message'])) {
                echo "<div class='alert alert-success'>".$_SESSION['message']."</div><br>";
                unset($_SESSION['message']);
               }

               if (isset($_SESSION['errorsDB'])) {
                echo "<div class='alert alert-danger'>".$_SESSION['errorsDB']."</div><br>";
               ...
               <form action="" method="post">
                 <div class="form-custom form-label form-border form-icon mb-3 bg-transparent">
                     ...
                     <input type="text" class="form-control rounded-xs" id="c1" name="username" placeholder="Username"/>
                     <label for="c1" class="color-theme">Username</label>
                     <span>(required)</span>
                 </div>
                 <div class="form-custom form-label form-border form-icon mb-3 bg-transparent">
                     <i class="bi bi-at font-16"></i>
                     <input type="email" class="form-control rounded-xs" id="c6" name="email" placeholder="Email Address"/>
                     <label for="c6" class="color-theme">Email Address</label>
                     <span>(required)</span>
                 </div>
                 <div class="form-custom form-label form-border form-icon mb-3 bg-transparent">
                 ...
                 <div class="form-custom form-label form-border form-icon mb-3 bg-transparent">
                     ...
                     <input type="password" class="form-control rounded-xs" id="c2" name="password" placeholder="Choose Password"/>
                 ...
                 <div class="form-custom form-label form-border form-icon mb-4 bg-transparent">
                     <i class="bi bi-asterisk font-13"></i>
                     <input type="password" class="form-control rounded-xs" id="c3" name="cpassword" placeholder="Confirm Password"/>
                 ...
                 <button type="submit" class="btn btn-full gradient-highlight shadow-bg shadow-bg-s mt-4">Create Account</button>
               </form>
               ...
               <div class="row">
                   ...
                   <div class="col-6 text-end">
                       <a href="login.php" class="font-11 color-theme opacity-40 pt-4 d-block">Sign In Account</a>
                   </div>
               </div>
           </div>
       </div>
